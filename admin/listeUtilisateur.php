<?php
// Application HSE
// Auteur : DELAUNAY Pierre
// Dernière mise à jour : 29/09/2017 par Pierre

// ouverture d'une session
session_start();  
// inclusion des paramètres et de la bibliothéque de fonctions ("include_once" peut être remplacé par "require_once")
include_once ('../include/_inc_parametres.php');
// connexion du serveur web à la base MySQL ("include_once" peut être remplacé par "require_once")
include_once ('../include/_inc_connexion.php');
// fonctions
include_once('../include/fonctions.php');

?>

<!DOCTYPE HTML>
<html>

<head>
  <title>Application HSE</title>
  <link rel="shortcut icon" href="../images/favicon.ico"/>
  <meta http-equiv="keywords" name="keywords" content="" />
  <meta http-equiv="description" name="description" content="" />
  <meta http-equiv="content-type" content="text/html; charset=utf8" />
  <link rel="stylesheet" type="text/css" href="../style/style.css" />
  <style type="text/css">

  /* ----------------- Mise en forme des tableaux -------------------- */

/* styles des tableaux */
table {
  /* pas d'espace entre les cellules */
  border-spacing: 0;
  
  /* largeur, marges et centrage horizontal */
  width: 700px;
  margin: 30px auto 30px auto;
  
  /* arrière-plan */
  background-color: rgb(230, 230, 230);
}

/* styles des tableaux ayant la propriété class="tableau" */
.tableau {
  /* bordure pour l'ensemble du tableau */
  border: 1px solid #000;
}


/* styles des cellules des tableaux ayant la propriété class="tableau" */
.tableau td, .tableau th {
  /* les cellules sont encadrés en haut et à gauche */
  border-left: 1px solid #000;
  border-top: 1px solid #000;
  padding: 5px;
  text-align: center;
}

  </style>

</head>

<body>
  <div id="main"> <!-- début id="main"-->

    <div id="header"> <!-- début id="header" -->
      <div id="logo">

      </div>

      <div id="menubar">
        <ul id="menu">
          <!-- class="selected" permet de mettre en évidence la page actuelle -->
          <li class="selected"><a href="gestionAdmin.php">Menu</a></li>
          <!--<li><a href="">Page 1</a></li>-->
          <li class="selected"><a href="../index.php?action=deconnexion">Déconnexion</a></li>
          <li class="selected"><a>Espace administration</a>
        </ul>
      </div>

    </div> <!-- fin id="header" -->

    <div id="site_content"> <!-- début id="site_content" -->
      <div id="content"> <!-- début id="content" -->
   
      <a href="gestionAdminAppli.php"><img src='../images/back.jpg' height='36' width='36'></a> 

      <h1>Liste des utilisateurs : </h1>


      <?php
      
      
      $resultat = $cnx->query("SELECT COUNT(*) AS Nb FROM hse_enseignants;");
      $resultat->setFetchMode(PDO::FETCH_OBJ);
        
      $ligne = $resultat->fetch();

      $countUtilisateurs = $ligne->Nb;

      if ($countUtilisateurs > 0) {
      /* On a bien des classes présentes dans la bdd, on affiche le tableau */
      
      $lesEnseignants = $cnx->query("SELECT enseignant_id AS ensID, enseignant_nom AS ensNom, enseignant_prenom AS ensPrenom FROM hse_enseignants ORDER BY enseignant_nom;");
      $lesEnseignants->setFetchMode(PDO::FETCH_OBJ);
      $unEnseignant = $lesEnseignants->fetch();
      
      echo "Total d'enseignants dans la base : ".$countUtilisateurs;
      
      /* Compteur pour l'affichage du total de classes présentes dans la liste */
      $compteurTotal = 0;

      // Entête du tableau

      echo "<table class='tableau'> ";
      echo "<thead>
              <tr>
                <td>ID</td>
                <td>Nom enseignant</td>
                <td>Prénom enseignant</td>
              </tr>
            </thead>";
      echo "<tr></tr>";

      while ($unEnseignant) {

      echo "<tr><td> ".($unEnseignant->ensID)."</td>";
      echo "<td> ".utf8_encode($unEnseignant->ensNom)."</td>";
      echo "<td> ".utf8_encode($unEnseignant->ensPrenom)."</td></tr>";
      $compteurTotal = $compteurTotal + 1;


      $unEnseignant = $lesEnseignants->fetch();

      }


      $lesEnseignants->closeCursor();

      echo "</table>";

      }
      else {
      /* Pas d'enseignants présents dans la bdd, on le signale à l'administrateur */
        echo "<p> Aucun enseignant n'est présent dans la base de données ! </p>";
      }



      ?>




      </div> <!-- fin id="content" -->
    </div> <!-- fin id="site_content" -->

    <div id="footer"> 
      <a href="http://www.lycee-delasalle.com/">Lycée De La Salle - RENNES</a>
    </div>

  </div> <!-- fin id="main" -->

</body>
</html>
